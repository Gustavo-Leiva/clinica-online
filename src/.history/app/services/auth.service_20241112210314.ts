...
async registrarUsuario(usuario: Usuario, password: string, file1?: File, file2?: File) {
  ...
  try {
    ...
    if (file1) {
      ...
      console.log('URL de fotoPerfil1:', fotoURL1); // Verifica si se obtiene la URL
    }
...
// Método para subir imágenes a Firebase Storage
  ...
  async subirImagen(file: File, path: string): Promise<string> {
    try {
      const storageRef = ref(getStorage(), path); // Obtener referencia a la ruta en Storage
      ...
      throw error; // Lanzar el error para manejarlo más arriba
    }
  }
