...
async registrarUsuario(usuario: Usuario, password: string, file1?: File, file2?: File) {
  ...
  try {
    ...
    let fotoURL1 = ''; // URL de la primera imagen de perfil
    let fotoURL2 = ''; // URL de la segunda imagen de perfil

    // Subir la primera imagen a Firebase Storage si `file1` está definido
    if (file1) {
      const path1 = `usuarios/${res.user.uid}/fotoPerfil1.jpg`;
      fotoURL1 = await this.subirImagen(file1, path1);
      console.log('URL de fotoPerfil1:', fotoURL1); // Verifica si se obtiene la UR
    }

    // Subir la segunda imagen a Firebase Storage si `file2` está definido
    if (file2) {
      const path2 = `usuarios/${res.user.uid}/fotoPerfil2.jpg`;
      fotoURL2 = await this.subirImagen(file2, path2);
      console.log('URL de fotoPerfil2:', fotoURL2); // Verifica si se obtiene la URL
    }

    // Preparar el objeto de datos para Firestore
    const userRef = doc(this.firestore, 'usuarios', res.user.uid); // Usamos el UID como ID del documento
    let userData;
    ...
    if (usuario.tipoUsuario === 'paciente') {
      ...
      userData = {
        ...paciente,
        uid: res.user.uid,
        fotoPerfil1: fotoURL1,  // Asigna la URL de la primera imagen de perfil
        fotoPerfil2: fotoURL2   // Asigna la URL de la segunda imagen de perfil
      };
    } else if (usuario.tipoUsuario === 'administrador') {
      const administrador = usuario as Administrador;
      userData = {
    ...
    console.log('Usuario guardado en Firestore con datos:', userData);
    ...
    let errorCode = e?.code || 'error-desconocido';
    this.msjError = this.mostrarErrorRegistro(errorCode);
    throw new Error(this.msjError);
  }
}
...
// Método para subir imágenes a Firebase Storage
// Método para subir imágenes a Firebase Storage
async subirImagen(image: File, path: string): Promise<string> {
  const storageRef = this.storage.ref(path);  // Crea la referencia a la ruta en Firebase Storage
  const task = await storageRef.put(image);  // Sube la imagen al Storage
  const downloadURL = await task.ref.getDownloadURL();  // Obtén la URL de descarga de la imagen
