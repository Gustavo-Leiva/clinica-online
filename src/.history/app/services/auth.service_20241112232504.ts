...
async registrarUsuario(usuario: Usuario, password: string, file1?: File, file2?: File) {
  ...
  try {
    ...
    let fotoPerfil1URL = ''; // URL de la primera imagen de perfil
    let fotoPerfil2URL = ''; // URL de la segunda imagen de perfil

    // Subir la imagen a Firebase Storage si file está definido
     // Subir la primera imagen de perfil
     if (file1) {
      const path1 = `usuarios/${res.user.uid}/fotoPerfil1.jpg`;  // Ruta de la primera imagen
      fotoPerfil1URL = await this.subirImagen(file1, path1);
      console.log('URL de la primera imagen subida:', fotoPerfil1URL);
    }

    // Subir la segunda imagen de perfil si es un paciente y si se proporciona
    if (file2) {
      const path2 = `usuarios/${res.user.uid}/fotoPerfil2.jpg`;  // Ruta de la segunda imagen
      fotoPerfil2URL = await this.subirImagen(file2, path2);
      console.log('URL de la segunda imagen subida:', fotoPerfil2URL);
    }
    ...
    if (usuario.tipoUsuario === 'paciente') {
      ...
      await setDoc(userRef, {
        ...paciente,
        uid: res.user.uid,
        fotoPerfil1: fotoPerfil1URL, // Agrega la URL de la primera imagen de perfil
        fotoPerfil2: fotoPerfil2URL  // Agrega la URL de la segunda imagen de perfil
...
async subirImagen(file: File, path: string): Promise<string> {
  ...
  await uploadBytes(storageRef, file);// La función `uploadBytes` sube el archivo a Firebase Storage en la ruta especificada
   // Después de que el archivo ha sido subido, obtenemos su URL de descarga
  const downloadURL = await getDownloadURL(storageRef);// `getDownloadURL` obtiene la URL pública del archivo subido
  
  return downloadURL; // Retorna la URL de descarga, la cual puedes guardar en Firestore o mostrar en la aplicación
}
