...
async registrarUsuario(usuario: Usuario, password: string, string, file?: File) {
  ...
  try {
    ...
    if (file) {
      const path = `usuarios/${res.user.uid}/fotoPerfil1.jpg`;  // Usa el UID de la respuesta para definir la ruta
